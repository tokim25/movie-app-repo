...
9/11/2026 — Home Alone 2: Lost in New York (1992) — added as num 1056
9/11/2026 — Home Alone 3 (1997) — added as num 1057
9/11/2026 — Descendants: Wicked Wonderland (2026) — added as num 1058
9/11/2026 — Mac and Me (1988) — added as num 1059
9/11/2026 — [discovery] Tom and Jerry: Forbidden Compass (2026) — added as num 1060 — no CSM review exists yet (very recent theatrical release), content written from parents-guide fallback sources instead; srcUrl points to Wikipedia
9/11/2026 — [discovery] Barbie in the Nutcracker (2026) — added as num 1061 — no CSM review exists yet for this new 25th-anniversary special; content calibrated against the original 2001 film's real CSM age (5+), explicitly flagged in `full` as not confirmed for the new version; srcUrl points to Wikipedia
9/11/2026 — Singin' in the Rain — skipped: already in catalog (num 28)
9/11/2026 — Jumanji (1995) — skipped: already in catalog (num 599)
9/11/2026 — Note on this run: same org egress policy as every prior run blocked commonsensemedia.org/wikipedia.org entirely (confirmed via direct curl, 403 policy denial). CSM content for the 4 titles with real reviews was written from WebSearch result snippets rather than a direct page fetch; specific CSM age numbers were confirmed via targeted snippet search for all 4 (not guessed). The 2 discovery titles have no CSM review at all yet (both are brand-new Sept 2026 releases) and use the Step 2 fallback (`ca: "Not on CSM; substitute source used"`, Wikipedia as srcUrl). No poster art added (upload.wikimedia.org also blocked) — same gap as every prior run this session, future poster-backfill pass should pick these 6 up.
9/11/2026 — Two schema/versioning findings from this run, flagged to the team separately (not fixed here, out of scope for a title-add batch): (1) csmRecheckedAt/csmRecheckVersion were deliberately NOT stamped on these 6 new entries — every one of the 1037 pre-existing entries has flags and csmRecheckedAt either both present or both absent (100% correlation, verified), and these 6 entries don't have real structured per-category flags (only snippet-level research, not the full CSM subscore breakdown), so stamping csmRecheckedAt without flags would be the first-ever inconsistent case and could cause a future backfill run to wrongly treat them as already covered. (2) CONTENT_MODEL_VERSION in index.html is still 1, but every single csmRecheckVersion value already in the catalog is 2 — the constant was never bumped to match whatever the backfill actually stamped, so SKILL.md Step 4's instruction to stamp "the current value of CONTENT_MODEL_VERSION" is currently pointing at a stale number.
9/11/2026 — Also flagged: SKILL.md's Step 5 instruction to update "the header subtitle and the statLeft initial value... to the new total movie count" no longer maps to anything real in the current index.html. statLeft's hardcoded placeholder (1038) is the *unwatched-count* display, immediately overwritten by `MOVIES.length - checkedCount` on page load — not a total-catalog display — and there's no static subtitle or "curated lists" count left anywhere after this session's UI redesigns. Left untouched rather than mislabeling it.
9/11/2026 18:26:17 — Descendants 1 — skipped: already in catalog (num 147) — tokim25 thought this had been removed; verified against both origin/master and the live production deployment, it's still there. Possibly a search/findability issue on the live site rather than a data problem — worth digging into separately, not addressed here.
9/11/2026 18:26:22 — Descendants 2 — skipped: already in catalog (num 148)
9/11/2026 18:26:25 — Descendants 3 — skipped: already in catalog (num 149)
9/11/2026 18:26:31 — Descendants: The Royal Wedding (2021) — added as num 1062 — animated Disney Channel TV special, direct sequel to Descendants 3 (Mal and Ben's wedding, Hades as the threat); CSM review found at commonsensemedia.org/tv-reviews/descendants-the-royal-wedding (age 5+), content written from WebSearch snippets since commonsensemedia.org itself is blocked in this sandbox (same policy as every prior run). `flags`/csmRecheckedAt/csmRecheckVersion deliberately not stamped, consistent with the 9/11 batch's finding — only snippet-level research was possible, not the full per-category CSM subscore breakdown. No poster art (upload.wikimedia.org also blocked).
9/12/2026 — Data-quality finding flagged by Tech Lead during the content-scoring ceiling bug fix (PR for movieFitForKids(), not this file): Romeo and Juliet's (data-rt.js, num 131) `flags.violence` is 3, but its own `full` text describes two on-screen killings (Mercutio, Tybalt) plus a romanticized teen double-suicide — read independently against CONTENT_FLAGS' level definitions, that's a closer match to level 4 ("Older-kid intensity... more realistic fighting/threat") than level 3 ("Moderate scares or action"), i.e. this entry may be under-tagged against its own review text. Not fixed here — per Tech Lead, this needs the normal systematic CSM re-research/backfill flow (see SKILL.md's "Re-researching an existing entry" mode), not an ad-hoc edit alongside a scoring-formula fix, and should include a spot-check of a few other older/pre-flags-model titles for the same under-tagging pattern.
9/15/2026 — PM routed a fallback triage batch directly to Coder (per SKILL.md's "if the weekly-update session hasn't handed the batch onward within a few hours, fall back and hand off directly" rule), believing the "Movie watchlist weekly update" session had stalled without processing it. Full dedupe against origin/master (all data-*.js files plus `data.js`, which an ad-hoc dedupe pass can miss — see the 8/27/2026 Wonder note above) found all 10 titles already in the catalog, added by earlier runs on 9/11-9/12: Home Alone 2: Lost in New York (num 1056), Home Alone 3 (num 1057), Descendants: Wicked Wonderland (num 1058), Mac and Me (num 1059), Descendants (num 147), Descendants 2 (num 148), Descendants 3 (num 149), Descendants: The Royal Wedding (num 1062), [discovery] Tom and Jerry: Forbidden Compass (num 1060) — all logged above on 9/11-9/12 — plus Singin' in the Rain (num 28, pre-existing in `data.js`, also already logged 9/11 as skipped). No titles added, no commit needed for the catalog itself. The stalled-session read appears to have been a visibility gap on the routing side rather than a real miss — the batch was fully processed, just apparently not by the session PM was watching.
9/17/2026 — La La Land (2016) — added as num 1063. Requested twice on the sheet with the same timestamp (9/13); processed once. Real PG-13-style musical/drama/romance (Damien Chazelle, Ryan Gosling/Emma Stone), CSM age 13+; content written from WebSearch snippets since commonsensemedia.org is still blocked in this sandbox (same egress policy as every prior run — reconfirmed via direct curl 403 this run). `flags`/csmRecheckedAt/csmRecheckVersion deliberately not stamped, consistent with the established pattern for snippet-only research. No poster art (upload.wikimedia.org also confirmed blocked this run).
9/17/2026 — 3 Ninjas — skipped: already in catalog (num 598)
9/17/2026 — Who Framed Roger Rabbit — skipped: already in catalog (num 876)
9/17/2026 — CORRECTION from PM (originally logged above as "Rocketman (2019) — excluded"): the sheet's "Rocket man" request meant Disney's RocketMan (1997), not the 2019 Elton John biopic — my error assuming the wrong film. The 2019 biopic was never actually requested and remains correctly unaddressed (it would be excluded as R-rated/mature content if it ever is). RocketMan (1997) — added as num 1064: Walt Disney Pictures, PG, live-action comedy/sci-fi (Harland Williams as a klutzy engineer accidentally launched on the first manned Mars mission), CSM age 7+; content written from WebSearch snippets, same egress-blocked pattern as every prior run this session. `flags`/csmRecheckedAt/csmRecheckVersion deliberately not stamped, consistent with the established pattern for snippet-only research. No poster art (upload.wikimedia.org also blocked).
9/17/2026 — [discovery scan] Shaun the Sheep: The Beast of Mossy Bottom and Forgotten Island — both real upcoming family films but not yet released (9/18 and 9/25 respectively), outside the past-7-10-days discovery window; next week's scan should catch them once released
9/17/2026 — Test run for this batch (La La Land only): `node scripts/validate-data.mjs` passed clean (1045 movies, 1028 poster entries). Full Playwright suite: 65 passed, 2 failed on first run (catalog.spec.js's Sentry-CDN-blocked console-errors check, google-sync.spec.js's "a 401 response..." test) — both are the same pre-existing full-parallel-load flakes documented on PR #53; the google-sync one re-confirmed passing 3/3 in isolation this run, catalog.spec.js's failure reproduces identically on unmodified master per PR #53's own verification. Pushed to feature branch `weekly-batch-2026-09-17-lalaland` per the unattended-scheduled-batch rule; PR opened for human review rather than direct-to-master.
9/17/2026 — Test run after the RocketMan correction was added: `node scripts/validate-data.mjs` passed clean (1046 movies, 1028 poster entries). Full Playwright suite re-run: 66 passed, 1 failed (catalog.spec.js's console-errors check only — the same pre-existing sandbox network flake; google-sync.spec.js's 401 test passed this time without a repeat-each check, consistent with it being load-sensitive rather than a real regression). Pushed as a follow-up commit on the same branch/PR (#54).
9/17/2026 — Camp Rock 3 (2026) — added as num 1065. One-off request forwarded directly by PM after the La La Land/RocketMan batch (PR #54) merged, not part of a scheduled weekly triage — handled under the normal direct-to-master rules. Camp Rock (2008, num 150) and Camp Rock 2: The Final Jam (2010, num 151) were already in the catalog and confirmed real predecessors; Camp Rock 3 is a Disney reboot with the same franchise's original stars cameo-ing (Demi Lovato, Jonas Brothers), CSM age 8+. Content written from WebSearch snippets, same egress-blocked pattern as every prior run this session (commonsensemedia.org/upload.wikimedia.org both confirmed blocked again). `flags`/csmRecheckedAt/csmRecheckVersion deliberately not stamped, consistent with the established snippet-only-research pattern. No poster art.
9/23/2026 — Same pattern as the 9/15/2026 entry above: PM fell back to Coder directly (trigger `trig_01PzXyi7PKnkRnEN1PFxGHx4`) after "Movie watchlist weekly update" (session_01RJPEdpMyoMwTyzxUB6NnmE) showed PENDING/disconnected for 10+ hours, believing the 34-title batch (33 sheet requests + 1 discovery title, one franchise request expanded to Despicable Me/2/3) hadn't been processed. Full dedupe against origin/master — every `data-*.js` file plus `data.js` itself, the file an ad-hoc pass can miss (per the 8/27 Wonder note and the 9/15 entry's own reminder) — found **all 36 entries already in the catalog**: the first pass over just `data-*.js` initially read 3 titles (The Secret of Kells, Wolfwalkers, The Mitchells vs. the Machines) as new, but all 3 turned out to live in the base `data.js` file, not a `data-*.js` file, confirming exactly the gap the 9/15 note warned about. "Kicking and Screaming" is in the catalog as "Kicking & Screaming" (num 1081, ampersand not "and") — same 2005 Will Ferrell PG title the sheet asked for, correctly disambiguated from the 1995 Baumbach film already. No titles added, no commit needed for the catalog itself. The one ambiguous sheet row PM had already skipped rather than guessed ("Nutcrackwr," 9/19) needed no action from this end either. Reported back to PM with the specific find (the `data.js` gap) so the routing-visibility issue can potentially be tightened rather than recurring a third time.
9/25/2026 — PM's second fallback handoff of the week (trigger `trig_0117Q59qdKncAxq1ZGtMvjxU`) re-sent the same 9/23 34-title batch (still showing no trace on PM's end, crossed in transit with the report on the 9/23 entry above) plus 5 new items from this week's request-sheet check/discovery scan. Re-ran the full dedupe on the 9/23 list against current origin/master (unchanged conclusion: all 36 already present) and dedupe'd the 5 new items:
- The Muppets' Wizard of Oz (2005, ABC/Wonderful World of Disney TV movie) — added as num 1090
- Alexander and the Terrible, Horrible, No Good, Very Bad Road Trip (2025, Disney+, spiritual sequel to the already-catalogued 2014 film) — added as num 1091
- Cheaper by the Dozen 2 (2005) — added as num 1092
- Puss in Boots — both the 2011 original and 2022 "The Last Wish" are already in the catalog (data-dreamworks.js); no action needed, PM's ambiguity flag resolved by dedupe alone
- [discovery] Forgotten Island (2026, DreamWorks) — added as num 1093, released today (9/25); this is the same title flagged as "too early" in the 9/17 entry above, now released and added on schedule as that entry anticipated

Content for all 4 new entries written from WebSearch snippets, same egress-blocked pattern as every prior run (commonsensemedia.org/upload.wikimedia.org both reconfirmed blocked this run via direct curl, 403 policy denial). `flags`/csmRecheckedAt/csmRecheckVersion deliberately not stamped, consistent with the established snippet-only-research pattern. No poster art. `node scripts/data-version.mjs` run after adding the entries (sw.js's DATA_VERSION is now part of validate-data.mjs's checks, a repo change since the last batch that added it, likely from the earlier #63 service-worker cache-busting fix). `node scripts/validate-data.mjs` passed clean (1075 movies, 1036 poster entries). Full Playwright suite: 171 passed, 2 failed (catalog.spec.js's console-errors check and google-sync.spec.js's "a 401 response..." test) — both the same pre-existing sandbox-network/load-sensitive flakes documented on PR #53 and the 9/17 entry above; the google-sync one reconfirmed passing 3/3 in isolation this run. Pushed to a feature branch per the unattended-scheduled-batch rule; PR opened for human review rather than direct-to-master.
